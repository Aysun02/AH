...
namespace ConsoleApp30
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("1.Icki novu daxiletme");
            Console.WriteLine("2.Satis");
            Console.WriteLine("3.Kassadaki meblegi gor");
            Console.WriteLine("4.Menuyu gor");
            ...
            foreach (var item in _products)
            {
                if (item.Name == name)
                {
                    if (item.Count < count)
                    {
                        if (item.Count != 0)
                        {
                            Console.WriteLine($"Sadece {item.Count} qeder var, bu qederisteyirsiniz?");
